...
Email addresses and telephone numbers go through `Obfuscate`: every character is
written as `&#NN;`, so the plain string never appears in the source and the
regular expressions address harvesters run come back empty.

It is a speed bump, not a lock. Anything that renders the page reads them
perfectly well, and nothing on a public page can prevent that; what it stops is
the cheap, high-volume end. It is entities rather than JavaScript assembly
because a business card has to work without JavaScript — the HTML parser decodes
them, so the link works, the text renders, and copy and paste gives the real
value. Every character is encoded rather than a random half, the way core's
`antispambot()` does it, so there is nothing left in the string to escape and no
"escaped except where it wasn't" to reason about.

The vCard is not obfuscated: it is a file somebody asked to download, and a
contact record full of character references would be useless.
...
## The quick actions are not fields

The four tiles at the top — call, email, WhatsApp, website — are the contact
details the card already carries, promoted. Two places to type the same telephone
