...
final class VCard {
	...
	private static function build( \WP_Post $post, array $card ): string {
		...
		foreach ( $telephones as $key => $types ) {
			...
			// quick_tel is usually one of the numbers above repeated as the
			// card's main button; a contact with the same number twice is
			// nobody's idea of helpful.
		...
		$website = $card['contact_website'] ?? ( $card['quick_website'] ?? '' );
